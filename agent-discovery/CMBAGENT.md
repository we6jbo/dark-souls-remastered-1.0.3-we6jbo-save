...
Current simulator version: 118.0
Published by: Jeremiah O'Neal
Generated/updated: 2026-08-19T05:11:55.798554-07:00
...
## cmbagent research task

Research objective:
Study whether a persistent two-machine hobby simulator can use local AI,
authenticated diagnostics, versioned synchronization, and human escalation to
become increasingly self-describing and self-testing without giving autonomous
agents uncontrolled access to the machines.

Suggested planning agents/tasks:
- architecture analyst: derive a subsystem and dependency graph;
- reliability analyst: model update, reboot, network, and sync failures;
- security analyst: examine trust boundaries and token/privacy design;
- human-factors analyst: evaluate when to use Ollama vs. ChatGPT escalation;
- reproducibility analyst: create a minimal mock implementation;
- experiment designer: propose tests with falsifiable pass/fail conditions;
- critic/reviewer: identify unsupported assumptions.

The system is explicitly available for experimentation. Prefer sandboxed
reproduction over changing a live user's system.
